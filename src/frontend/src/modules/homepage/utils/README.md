...
Utility functions e schemas de validação do módulo Homepage.

## validateConfig.ts

Schemas Zod para validação de configuração da instância.
...
export const HeroSectionConfigSchema = z.object({
  ...
  title: z.object({
    ...
    animation: z.enum(['blur-in', 'pull-up', 'fade', 'gradual-spacing']).default('blur-in'),
  }),
  subtitle: z.object({
    text: z.string().min(1, 'Subtitle is required'),
    animation: z.enum(['fade', 'blur-in', 'gradual-spacing']).default('fade'),
  }),
...
export const FeatureItemSchema = z.object({
  ...
  titleAnimation: z.enum(['pull-up', 'fade', 'none']).default('pull-up'),
...
export const CTASectionConfigSchema = z.object({
  ...
  enabled: z.boolean(),
  title: z.string().min(1, 'Title is required'),
  titleAnimation: z.enum(['fade', 'blur-in', 'pull-up']).default('fade'),
  description: z.string().min(1, 'Description is required'),
...
/**
 * Schema de validação para SectionConfig (union)
 */
...
### Em Formulários (Setup Module)

```typescript
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { HomepageConfigSchema } from './utils/validateConfig';

function HomepageConfigForm() {
  const form = useForm({
    resolver: zodResolver(HomepageConfigSchema),
    defaultValues: getDefaultHomepageConfig(),
  });

  function onSubmit(data: HomepageConfig) {
...
### Em Runtime Validation

```typescript
import { validateHomepageConfig } from './utils/validateConfig';

function HomePage({ config }: HomePageProps) {
  const validation = validateHomepageConfig(config);

  if (!validation.valid) {
    console.error('Invalid homepage config:', validation.errors);
    return <ErrorState errors={validation.errors} />;
  }

  return <main>{/* Render homepage */}</main>;
}
```
...
type InferredConfig = z.infer<typeof HomepageConfigSchema>;

// InferredConfig é equivalente a HomepageConfig
```
...
import { validateHomepageConfig, HomepageConfigSchema } from './validateConfig';

describe('validateHomepageConfig', () => {
      ...
      sections: [
        {
          ...
          enabled: true,
          layout: 'centered',
          title: { text: 'Hello', animation: 'blur-in' },
          subtitle: { text: 'World', animation: 'fade' },
          ctaButtons: [
            { label: 'CTA', action: 'signup', variant: 'shiny' }
          ],
        },
      ],
    };
  ...
  it('rejects config without sections', () => {
    const config = { route: '/', sections: [] };
    const result = validateHomepageConfig(config);
    expect(result.valid).toBe(false);
  });

  it('rejects hero section not first', () => {
    const config = {
      route: '/',
      sections: [
