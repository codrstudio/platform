/**
 * BlueprintHeaderConfigForm Component
 * DESIGN Reference: DES-FORM-001, DES-FORM-002
 *
 * Configuration form for BlueprintHeader slot component.
 * Allows editing menu items, toggles for logo and theme switcher.
 */
...
import {
  blueprintHeaderConfigSchema,
  type BlueprintHeaderConfig,
  type MenuItem,
...
export function BlueprintHeaderConfigForm({
  ...
  // Merge with defaults
  const currentConfig: BlueprintHeaderConfig = {
    ...defaultBlueprintHeaderConfig,
    ...config,
  };

  const [menuItems, setMenuItems] = useState<MenuItem[]>(currentConfig.menuItems);
  const [showBrandLogo, setShowBrandLogo] = useState(currentConfig.showBrandLogo);
  const [showThemeToggle, setShowThemeToggle] = useState(currentConfig.showThemeToggle);
  const [sticky, setSticky] = useState(currentConfig.sticky);

  // Propagate changes to parent
  const propagateChanges = (updates: Partial<BlueprintHeaderConfig>) => {
    const newConfig = {
      menuItems,
      showBrandLogo,
      showThemeToggle,
    ...
    // Validate with Zod
    const result = blueprintHeaderConfigSchema.safeParse(newConfig);
    if (result.success) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure general behavior of the header
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <Label htmlFor="show-logo" className="flex-1">
              <div className="font-medium">Show Brand Logo</div>
              <p className="text-sm text-muted-foreground">
                Display the brand logo on the left side
              </p>
            </Label>
            <Switch
              id="show-logo"
              ...
              onCheckedChange={(checked) => {
                ...
                propagateChanges({ showBrandLogo: checked });
              }}
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="show-theme" className="flex-1">
              <div className="font-medium">Show Theme Toggle</div>
              <p className="text-sm text-muted-foreground">
                Display the theme switcher button
              </p>
            </Label>
            <Switch
              id="show-theme"
              ...
              onCheckedChange={(checked) => {
                ...
                propagateChanges({ showThemeToggle: checked });
              }}
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="sticky" className="flex-1">
            ...
            <Switch
              ...
              onCheckedChange={(checked) => {
                setSticky(checked);
                propagateChanges({ sticky: checked });
              }}
            />
          </div>
        </CardContent>
