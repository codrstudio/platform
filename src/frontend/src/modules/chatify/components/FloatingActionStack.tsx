...
export function FloatingActionStack() {
  ...
  const { completionPercentage, widgetDismissed, settings, undismissWidget } = useJourneyProgress()
  const { toggle: toggleWidget } = useNextStepWidget()
  const { messages, isLoading, sendMessage, cancelMessage, startNewChat, unreadInsightsCount, markInsightsAsRead } = useChatify()
