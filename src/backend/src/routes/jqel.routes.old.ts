...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (schema === 'system') {
      // System schema - check if it's SDL query
      if (isSelect && query.select === 'sdl') {
        return await handleBackendSchema(query, res)
      }
      // Other system queries - route to n8n
      return await handleN8nSchema(query, res)
    }

    if (schema === 'platform') {
      // Platform schema - route to n8n
      return await handleN8nSchema(query, res)
    }
...
async function handleBackendSchema(query: JQELQuery, res: Response) {
  try {
    ...
    const normalizedEntity = entity!.replace(/s$/, '') as 'portal' | 'module' | 'instance' | 'realm' | 'login-branding' | 'sdl'

    if (isSelect) {
      return await handleSelect(query, normalizedEntity, res)
    } else {
      // SDL is read-only, no mutations allowed
      if (normalizedEntity === 'sdl') {
        return res.status(400).json({
          code: 400,
 ...
 */
async function handleSelect(query: JQELQuery, entity: 'portal' | 'module' | 'instance' | 'realm' | 'login-branding' | 'sdl', res: Response) {
