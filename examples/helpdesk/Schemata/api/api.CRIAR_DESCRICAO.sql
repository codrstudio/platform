-- Procedure para criar descrições (metadata) para qualquer tipo de objeto do SQL Server
-- Suporta: tabelas, views, procedures, functions, triggers, indexes, schemas, etc.
-- Idempotente: não cria se já existe
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[api].[CRIAR_DESCRICAO]') AND type in (N'P', N'PC'))
BEGIN
    EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [api].[CRIAR_DESCRICAO] AS'
END
GO

ALTER PROCEDURE [api].[CRIAR_DESCRICAO]
    ...
    @descricao NVARCHAR(3000),      -- Descrição a ser adicionada
...
BEGIN
    ...
    -- Remove descrição existente se houver
    IF EXISTS (
    ...
    DECLARE @msg NVARCHAR(500) = 'Descrição criada para ' +
