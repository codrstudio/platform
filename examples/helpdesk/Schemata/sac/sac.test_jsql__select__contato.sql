-- =============================================
-- Procedure: sac.test_jsql__select__contato
-- TESTE REAL v4: Detecta bugs de OFFSET com múltiplos filtros
-- USA: id_cliente + ativo + LIMIT + OFFSET + ORDER BY
-- =============================================

IF OBJECT_ID(N'sac.test_jsql__select__contato', N'P') IS NULL
BEGIN
    EXEC('CREATE PROCEDURE sac.test_jsql__select__contato AS SELECT 1');
END
GO

ALTER PROCEDURE sac.test_jsql__select__contato
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Usa id_cliente + ativo + offset para forçar código bugado
    -- =============================================
    BEGIN TRY
        EXEC sac.jsql__select__contato
            @jsql = N'{"select":"contato","where":{"id_cliente":{"gt":0},"ativo":{"eq":true}},"options":{"limit":5,"offset":1,"orderBy":"id_contato","orderDirection":"DESC"}}',
        ...
        INSERT INTO #test_results VALUES (
            'TC1: id_cliente+ativo+OFFSET',
            CASE WHEN @error IS NULL THEN 'PASS' ELSE 'FAIL' END,
            @error
        );
    END TRY
    BEGIN CATCH
        INSERT INTO #test_results VALUES ('TC1: id_cliente+ativo+OFFSET', 'FAIL', ERROR_MESSAGE());
    END CATCH
    ...
    PRINT '================================================';
    PRINT 'SUMÁRIO: sac.test_jsql__select__contato';
    PRINT '================================================';
