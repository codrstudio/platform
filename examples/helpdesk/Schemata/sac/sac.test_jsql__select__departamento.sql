-- =============================================
-- Procedure: sac.test_jsql__select__departamento
-- TESTE REAL v4: Detecta bugs de OFFSET com múltiplos filtros
-- USA: id_departamento + ativo + LIMIT + OFFSET + ORDER BY
-- =============================================

IF OBJECT_ID(N'sac.test_jsql__select__departamento', N'P') IS NULL
BEGIN
    EXEC('CREATE PROCEDURE sac.test_jsql__select__departamento AS SELECT 1');
END
GO

ALTER PROCEDURE sac.test_jsql__select__departamento
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Usa id_departamento + ativo + offset para forçar código bugado
    -- =============================================
    BEGIN TRY
        EXEC sac.jsql__select__departamento
            @jsql = N'{"select":"departamento","where":{"id_departamento":{"gt":0},"ativo":{"eq":true}},"options":{"limit":5,"offset":1,"orderBy":"id_departamento","orderDirection":"DESC"}}',
        ...
        INSERT INTO #test_results VALUES (
            'TC1: id_departamento+ativo+OFFSET',
            CASE WHEN @error IS NULL THEN 'PASS' ELSE 'FAIL' END,
            @error
        );
    END TRY
    BEGIN CATCH
        INSERT INTO #test_results VALUES ('TC1: id_departamento+ativo+OFFSET', 'FAIL', ERROR_MESSAGE());
    END CATCH
    ...
    PRINT '================================================';
    PRINT 'SUMÁRIO: sac.test_jsql__select__departamento';
    PRINT '================================================';
