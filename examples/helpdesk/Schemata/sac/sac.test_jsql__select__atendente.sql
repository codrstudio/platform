-- =============================================
-- Procedure: sac.test_jsql__select__atendente
-- TESTE REAL v4: Detecta bugs de OFFSET com múltiplos filtros
-- USA: id_usuario + ativo + LIMIT + OFFSET + ORDER BY
-- =============================================

IF OBJECT_ID(N'sac.test_jsql__select__atendente', N'P') IS NULL
BEGIN
    EXEC('CREATE PROCEDURE sac.test_jsql__select__atendente AS SELECT 1');
END
GO

ALTER PROCEDURE sac.test_jsql__select__atendente
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Usa id_usuario + ativo + offset para forçar código bugado
    -- =============================================
    BEGIN TRY
        EXEC sac.jsql__select__atendente
            @jsql = N'{"select":"atendente","where":{"id_usuario":{"gt":0},"ativo":{"eq":true}},"options":{"limit":5,"offset":1,"orderBy":"id_atendente","orderDirection":"DESC"}}',
        ...
        INSERT INTO #test_results VALUES (
            'TC1: id_usuario+ativo+OFFSET',
            CASE WHEN @error IS NULL THEN 'PASS' ELSE 'FAIL' END,
            @error
        );
    END TRY
    BEGIN CATCH
        INSERT INTO #test_results VALUES ('TC1: id_usuario+ativo+OFFSET', 'FAIL', ERROR_MESSAGE());
    END CATCH
    ...
    PRINT '================================================';
    PRINT 'SUMÁRIO: sac.test_jsql__select__atendente';
    PRINT '================================================';
